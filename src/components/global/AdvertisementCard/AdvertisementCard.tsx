    ...
    >
      <View style={{ position: 'relative' }}>
        ...
        >
          ...
          {/* Price */}
          <View className="pt-2">
            <Text className="text-xl font-bold text-font dark:text-font-dark">{getFormattedPrice()}</Text>
          </View>
        </View>
        ...
        {item.images && item.images.length > 0 && (
          <View className="px-4 pb-3">
            <FlashList
              ...
              renderItem={({ item: image, index: _index }) => <ImageItem imageUri={DefaultConfig.basePath + image} />}
            />
          </View>
        )}
        ...
        <View className="flex-row items-center justify-between px-4 py-3">
          {item.region && (
            <View className="flex-row items-center">
              ...
              <Text className="ml-1 text-sm text-font-subtlest dark:text-font-subtlest-dark">{item.region}</Text>
            </View>
          )}
          {item.createdAt && <Text className="text-xs text-font-subtlest dark:text-font-subtlest-dark">{getFormattedDate()}</Text>}
        </View>
      </View>
    </RectButton>
  );
