  ...
  <View className="flex-row items-center justify-between py-1">
    ...
    <Text
      className={`flex-1 text-right text-base ${
        highlighted ? 'font-medium text-font-brand dark:text-font-brand-dark' : 'text-font dark:text-font-dark'
      }`}
    >
      {value}
    </Text>
  ...
  if (error) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-background-page px-4 dark:bg-background-page-dark">
        ...
        <TouchableOpacity className="rounded-xl bg-background-success-bold px-6 py-3 dark:bg-background-success-bold-dark" onPress={onBack}>
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} onScroll={handleScroll} scrollEventThrottle={16}>
        ...
        <View className="relative" style={{ width, height: 300 }}>
          {productData.imageUri ? (
            <Image source={{ uri: productData.imageUri }} className="h-full w-full" resizeMode="cover" />
          ) : (
        ...
        <View className="p-4">
          ...
          <Text className="text-sm text-font-subtlest dark:text-font-subtlest-dark">{productData.region}</Text>
        </View>

        {/* Description */}
        {productData.description && (
          <View className="mx-4 mt-2 rounded-2xl bg-surface p-5 dark:bg-surface-dark">
            <Text className="mb-3 text-xl font-semibold text-font dark:text-font-dark">Описание</Text>
            <Text className="text-base leading-6 text-font-subtle dark:text-font-subtle-dark">{productData.description}</Text>
          </View>
        )}
          ...
          <View className="mx-4 mt-2 rounded-2xl bg-surface p-5 dark:bg-surface-dark">
            ...
            <View className="gap-3">
              {displaySpecs.map((spec, index) => (
                <SpecificationRow key={`${spec.label}-${index}`} label={spec.label} value={spec.value} highlighted={spec.highlighted} />
              ))}
            </View>
